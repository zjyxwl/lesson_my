const data = [
    { name: "questions", value: 17 },
    { name: "schools", value: 25 },
    { name: "philosophers", value: 35 },
];
// canvas 画布
const charWidth = 480;   // 条形图的宽度
const charHeight = 300;  // 条形图的高度
const margin = 15;       // 条形图的外间距

const containerWidth = charWidth + margin*2;
const containerHeight = charHeight + margin*2;
// 取出所有的names x坐标
// []   new Array[]  Array.from
const names = Array.from(data, (d) => d.name);
// console.log(names)
const values = Array.from(data, (d) => d.value);
console.log(values)
// 不需要item _ 占位
const indices = Array.from(data, (_,i) => i);   // _ 占位符
console.log(indices)

const step = charWidth / names.length;   // 执行几步
const barWidth = step * 0.8   // x轴偏移
const xs = Array.from(indices, (i) => i * step)

const y = charHeight;
const vmax = Math.max(...values)
const barHeights = Array.from(values, (v) => charHeight * (v / vmax))
console.log(barHeights)

const nameColor = {
    questions: '#5B8FF6',
    philosophers: '#61DDAA',
    schools: '#65789B'
}

const colors = Array.from(names, (name) => nameColor[name])
console.log(colors)